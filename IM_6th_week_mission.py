...
class Dragon(Monster):
    def move(self):
        print(f'[{self.name}] 하늘 날기!')

wolf = Wolf('울프링크', 50, 10)
wolf.move()

baby_shark = Shark('아기상어', 10000, 5000)
baby_shark.move()

D = Dragon('투슬리스', 50000, 10000)
D.move()
'''
...
class Dragon(Monster):
    def __init__(self,name,health,attack):
        ...
        self.skills = ['火炎放射', '泣く', 'フレア']

    def move(self):
        print(f'[{self.name}] 하늘 날기!')

    def skill(self):
        print(f'{self.name}) [{self.skills[random.randint(0,2)]}] スキルを使った!！効果は素晴らしい！')

wolf = Wolf('ウルフリンク', 50, 10)
print(f'늑대 생성 후: {wolf.max_num}')

baby_shark = Shark('아기상어', 10000, 5000)
print(f'상어 생성 후: {baby_shark.max_num}')

D = Dragon('ツスリス', 50000, 10000)
print(f'드래곤 생성 후: {D.max_num}')
D.skill()

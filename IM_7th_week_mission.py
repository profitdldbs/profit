...
'''
num1 = float(input('첫번째 숫자를 입력해주세요.(숫자를 입력해주세요)>> '))
num2 = float(input('첫번째 숫자를 입력해주세요.(숫자를 입력해주세요)>> '))
print(num1+num2)
'''
## 예외처리 연습문제: 환율 계산 문제
'''
...
## 에러 발생시키기 연습문제
'''
kka_ak = input('문자열을 입력해주세요>> ')
if kka_ak == '어물전 망신은':
    raise Exception('...개망신')
'''
## 예외처리 Mission: 영어단어 공부하기 문제를 'while문 + 예외 처리'로 풀어보자
'''
words = ['Oh Soo Jin', 'Pneumonoultramicroscopicsilicovolcanoconiosises', 'Supercalifragilisticexpialidocious']
i=0
count = 0
while True:
    try:
    ...
    if temp == words[i]:
        count += 1
    i += 1
print(f'전체개수: {len(words)}')
print(f'맞은개수: {count}')
print(f'틀린개수: {len(words)-count}')
'''
# [파일 입출력]
